...
**default 插槽**（插槽）：放置表单控件（OInput、OSelect 等）。OForm 提供 `--form-widget-width-standard` 等响应式宽度变量，若项目全局样式已统一应用（见「全局调用建议」），则模板中无需再设置控件宽度。OButton 宽度不在变量管理范围内，由调用方自行决定。
...
- **⚠️ 多列表单布局**：设计稿中表单项并排显示时，使用 `OForm layout="h"`（保持单列堆叠，标签左置）。OForm 提供 `--form-widget-width-standard`/`--form-widget-width-wide` 等响应式变量，**推荐在项目全局样式中统一应用**（见「全局调用建议」），模板中无需对每个控件单独设 width（OButton 除外）。
  - 若项目设计稿与默认值不符，全局覆盖变量即可：`.o-form { --form-widget-width-standard: xxx; --form-widget-width-wide: xxx; }`
...
.o-form {
  .o-input, .o-select, .o-input-number, .o-cascader, .o-ip-input {
    width: var(--form-widget-width-standard);
  }
  .o-textarea {
    width: var(--form-widget-width-wide);
  }
}
```
...
|---------|------------|----------------------|--------|
| `--form-widget-width-standard` | `--o-r-grid-6` | `--o-r-grid-4` | `100%` |
| `--form-widget-width-wide` | `--o-r-grid-14` | `--o-r-grid-8` | `100%` |
| `--form-widget-inline-gap` | `--o3-gap-4` | — | — |
| `--form-widget-width-min` | `(standard - gap) / 2` | — | — |

若项目设计稿与默认值不符，全局覆盖变量即可：
```css
.o-form {
  --form-widget-width-standard: 280px;
  --form-widget-width-wide: 560px;
}
```
...
.o-form {
  ...
  .form-inline {
    display: flex;
    align-items: center;
    gap: var(--form-widget-inline-gap);

    .o-input, .o-select, .o-input-number {
      width: var(--form-widget-width-min);
      flex: 1 1 var(--form-widget-width-min);
      min-width: 0;
    }
...
<template>
  <OForm has-required layout="h" label-width="96px" :model="formData">
    ...
    <!-- OInput / OSelect 等：全局样式已应用 --form-widget-width-standard，此处无需写 width -->
    ...
    <!-- OTextarea：全局样式已应用 --form-widget-width-wide（较宽），label-align="top" 避免标签悬空 -->
