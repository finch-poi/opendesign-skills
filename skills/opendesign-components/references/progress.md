...
🧩 **布局结构**：线形进度条水平排列，从左到右为：轨道区（含进度条填充和可选内部文字）、标签区（百分比文字或图标，仅 labelInside=false 时在右侧显示）。环形进度条为 SVG 圆环，中心叠加标签文字。轨道圆角等于线宽，标签间距 8px。
```yaml
# 简化结构摘要（完整版见 Part B）
...
- **Token → Prop 映射**：蓝色/品牌色进度条 → color="primary"；绿色 → color="success"；橙色 → color="warning"；红色 → color="danger"；线宽 8px → size="medium"（默认）；线宽 4px → size="small"；文字在进度条内部 → labelInside；圆环形 → variant="circle"
...
3. 进度条内部嵌入白色百分比文字 → 匹配 OProgress（line + labelInside）
...
| 文字位置 | 进度条内部 | labelInside | `true` | 仅 line 模式 |
