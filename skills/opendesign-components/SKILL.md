...
OpenDesign 是一个面向 openEuler 生态的 Vue 3 组件库，提供 59 个可复用 UI 组件。组件库有三套独立主题，**每个社区项目在初始化时选定一套，运行时只切换 dark/light 模式**。
...
三套主题对应不同社区，**项目初始化时选定其中一套**，引入对应的 token 文件：

**openEuler 社区项目：**
```typescript
// main.ts
import '@opensig/opendesign-token/themes/e.token.css'  // openEuler 主题 token
...
```typescript
// main.ts
import '@opensig/opendesign-token/themes/a.token.css'  // Ascend 主题 token
import '@opensig/opendesign/es/index.scss'
```
...
```typescript
// main.ts
import '@opensig/opendesign-token/themes/k.token.css'  // Kunpeng 主题 token
import '@opensig/opendesign/es/index.scss'
```

> **注意**：引入顺序很重要，必须先引入 token.css，再引入组件样式（`.scss`）。
...
简单数据表格，通过 `columns` 和 `data` 属性定义结构和数据。

**主要属性**：

| 属性 | 类型 | 默认 | 说明 |
|------|------|------|------|
| `columns` | `TableColumnT[] \| string[]` | — | 列配置 |
| `data` | `TableRowT[]` | — | 表格数据 |
| `border` | `'all' \| 'row' \| 'column' \| 'frame' \| 'row-column' \| 'row-frame' \| 'column-frame' \| 'none'` | `'row'` | 边框样式 |
| `stripe` | `boolean` | `false` | 斑马纹 |
| `small` | `boolean` | — | 小尺寸 |
| `loading` | `boolean` | — | 加载状态 |
| `emptyLabel` | `string` | — | 空数据提示文本 |
| `highlightCurrentRow` | `boolean` | `false` | 高亮当前行 |
| `cellSpan` | `Function` | — | 单元格合并函数 |

**插槽**：
- `#header` — 自定义表头（支持多行表头），`props: { columns }`
- `#th_{columnKey}` — 自定义某列的列头，`props: { column }`
- `#td_{columnKey}` 或 `#{columnKey}` — 自定义某列单元格，`props: { row, rowIndex }`
- `#empty` — 空状态
- `#loading` — 加载状态
