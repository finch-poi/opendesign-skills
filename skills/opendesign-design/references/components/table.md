...
│   └── [排序图标 SYMBOL]（24×24，componentKey 见下）
├── [标题3 PARAGRAPH]（42px，SemiBold 16/24，color-info2）
├── [筛选图标 SYMBOL]（24×24，componentKey 见下）
...
│   │   ├── [标题2 text] + [排序图标 SYMBOL]
│   ├── [标题3 PARAGRAPH]（1370:895，42px）
│   ├── [筛选图标 SYMBOL]（1370:896，24×24）
...
| 图标 | 用途 | componentKey |
|------|------|-------------|
| 排序 Icon/排序 | 列头排序触发 | `cdc0445a267b87be40951bdef568a94c5032e299` |
| 筛选 Icon/筛选 | 列头筛选触发 | `f018d0b82276d631b4d27b92bae9cc3df5197ee6` |

> 图标来源：Pixso 文件 `ZuTufm9KC1JTIM-KwWz52A`（图标库），尺寸 24×24px，颜色继承 `color-info2`。
...
2. **添加排序图标**：列头标题旁 `create_instance(cdc0445a267b87be40951bdef568a94c5032e299)`，尺寸 24×24，颜色 `color-info2`
3. **添加筛选图标**：`create_instance(f018d0b82276d631b4d27b92bae9cc3df5197ee6)`，同上
...
- **操作按钮宽度固定 64px**：文字居中对齐，使用 `color-link1`，不加下划线
...
1. **列宽分配**：主内容列（第一列）使用"自适应宽度"（Fill），其余列固定宽度；操作列固定 64px
2. **操作列数量**：建议不超过 3 个操作，超出时用"更多"下拉
3. **斑马纹使用**：行数较多（>8 行）时建议开启斑马纹，提升扫读体验
4. **排序列优先级**：最多 1 列同时激活排序状态，避免歧义
5. **复选框表头**：行选择模式下，表头首列也应放置 Mix（全选）状态 Checkbox
