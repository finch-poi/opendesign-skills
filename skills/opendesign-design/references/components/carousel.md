...
# OCarousel 走马灯 · 设计 Skill

> 组件集合节点：`1042:21288` · 组件名：OCarousel 走马灯指示器 · 变体总数：4（size × Dark）
...
**OCarousel 走马灯指示器**：用于走马灯（轮播图）底部的位置指示组件，由多个横向排列的矩形条（指示块）组成。当前活跃项以品牌色高亮显示，其余项以半透明色示意，直观呈现当前所在位置及总项数。提供 `small` 和 `medium` 两种尺寸，支持浅色 / 深色主题。
