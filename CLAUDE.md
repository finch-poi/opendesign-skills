...
本文件提供对 Claude Code 在此仓库中工作的指导。

---

## 仓库用途

**OpenDesign Skills** 是为 AI 编码助手（Claude Code、Cursor 等）生成和维护的知识库。它包含 OpenDesign 生态（Vue 3 组件库、CLI 构建工具、设计令牌）的完整参考文档，用于：
...
├── LICENSE
├── CLAUDE.md                      ← 本文件，Claude Code 工作指导（第一层：生产指导）
...
└── skills/
    ...
    │       ├── tokens.md
    │       └── tokens-{theme}.md
    └── openeuler-frontend-tools/  ← 前端工具库参考
        ├── SKILL.md
        └── references/
            ├── composables.md
            ├── mixins.md
...
| 层级 | 名称 | 受众 | 典型文件 |
|------|------|------|---------|
| **第一层** | 生产指导 | Claude Code（写作者） | `CLAUDE.md`、`SKILL-GEN-GUIDE.md`、`SKILL-REVIEW-GUIDE.md`、`*.review.md`、`_skill-gen-status.md` |
